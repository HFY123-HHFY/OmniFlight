...
/*
 * 运行时修改 PWM 周期（ARR）。
 * 仅写 ARR 寄存器，适合无源蜂鸣器调频等需要动态改变频率的场景。
 */
void API_PWM_SetPeriod(API_PWM_Tim_t timId, uint16_t arr)
{
	const API_PWM_Config_t *config;
	...
	config = API_PWM_FindTimerConfig(timId);
	if (config == 0)
	{
		return;
	}
	...
	F407_PWM_SetARR(config->coreTimId, arr);
#else
	(void)config;
	(void)arr;
#endif
